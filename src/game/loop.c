...
/*   Updated: 2025/05/24 20:41:00 by sal-kawa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int load_texture(t_texture *tex, mlx_t *mlx, char *path, t_all_struct *cub_map)
{
    ...
    if (!tex->texture)
    {
        free_all(cub_map, 1, "Failed to load texture");
        return 0;
    }
    ...
    if (!tex->img)
    {
        ...
        free_all(cub_map, 1, "Failed to convert texture to image");
        return 0;
    }
...
void init_game(t_all_struct *cub_map)
{
    ...
    if (!cub_map->game.mlx)
        free_all(cub_map, 1, "Failed to initialize MLX42");

    cub_map->game.img = mlx_new_image(cub_map->game.mlx, WIDTH, HEIGHT);
    if (!cub_map->game.img)
        free_all(cub_map, 1, "Failed to create image");

    if (mlx_image_to_window(cub_map->game.mlx, cub_map->game.img, 0, 0) < 0)
        free_all(cub_map, 1, "Failed to put image to window");

    cub_map->game.map = cub_map->map.real_map_two_d;
    cub_map->game.all_struct = cub_map;

    if (!load_texture(&cub_map->game.north, cub_map->game.mlx, cub_map->map.path_north_no, cub_map))
        return;
    if (!load_texture(&cub_map->game.south, cub_map->game.mlx, cub_map->map.path_south_so, cub_map))
        return;
    if (!load_texture(&cub_map->game.east, cub_map->game.mlx, cub_map->map.path_east_ea, cub_map))
        return;
    if (!load_texture(&cub_map->game.west, cub_map->game.mlx, cub_map->map.path_west_we, cub_map))
        return;
